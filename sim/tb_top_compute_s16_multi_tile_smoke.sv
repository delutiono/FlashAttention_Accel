...
module tb_top_compute_s16_multi_tile_smoke;
  ...
  localparam int unsigned ROWS = 16;
  localparam int unsigned KV_TILE_ROWS = 4;
  ...
  initial begin : static_config_checks
    if (dut.COMPUTE_ROWS != ROWS) begin
      fail("top compute S16 smoke COMPUTE_ROWS override did not apply");
    end
    if (dut.KV_TILE_ROWS != KV_TILE_ROWS) begin
      fail("top compute S16 smoke KV_TILE_ROWS override did not apply");
    end
  end
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (m_axi_arvalid && m_axi_arready) begin
        if ((m_axi_araddr >= Q_BASE) && (m_axi_araddr < K_BASE)) begin
          ...
          if (m_axi_araddr >= (K_BASE + (ROWS * STRIDE_BYTES))) begin
          ...
          if (m_axi_araddr >= (V_BASE + (ROWS * STRIDE_BYTES))) begin
            saw_oob_kv_read   <= 1'b1;
            oob_kv_read_count <= oob_kv_read_count + 1;
          end
        end
      ...
      if (m_axi_awvalid && m_axi_awready &&
          (m_axi_awaddr >= O_BASE) &&
          (m_axi_awaddr < (O_BASE + (ROWS * STRIDE_BYTES)))) begin
  ...
  initial begin
    ...
    $readmemh("test_vectors/generated/s16_d64_seed102/s16_d64_seed102_Q_beats64.hex", q_beats);
    $readmemh("test_vectors/generated/s16_d64_seed102/s16_d64_seed102_K_beats64.hex", k_beats);
    $readmemh("test_vectors/generated/s16_d64_seed102/s16_d64_seed102_V_beats64.hex", v_beats);
    $readmemh("test_vectors/generated/s16_d64_seed102/s16_d64_seed102_O_golden_beats64.hex",
              o_golden_beats);
    ...
    do begin
      ...
      if (status[2]) fail("top reported compute S16 multi-tile smoke error");
      poll_count++;
      if (poll_count > 60000) fail("timeout waiting for top compute S16 multi-tile smoke done");
    end while (!status[1]);
    ...
    for (idx = 0; idx < TOTAL_BEATS; idx++) begin
      ...
      if (got_word !== o_golden_beats[idx]) begin
        $fatal(1,
               "S16 compute O beat %0d mismatch got=0x%016x expected=0x%016x",
    ...
    $display("tb_top_compute_s16_multi_tile_smoke PASS S=16 D=64 KV_TILE_ROWS=4");
    $finish;
  end
