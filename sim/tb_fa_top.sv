...
module tb_fa_top;
  ...
  // =============================================================
  //  X-assertion: detect X on DMA write master critical signals
  //  These would cause m_axi_wlast=X → AXI burst stuck → timeout
  // =============================================================
  always @(posedge clk) begin
    if (wrm_state == 2'd2) begin  // ST_W
      if ($isunknown(wrm_beat_in))
        $display("[%0t] *** ASSERTION FAIL: beat_in_burst_reg is X (4'b%0b) in ST_W!", $time, wrm_beat_in);
      if ($isunknown(wrm_burst_beats))
        $display("[%0t] *** ASSERTION FAIL: burst_beats_reg is X (5'b%0b) in ST_W!", $time, wrm_burst_beats);
    end
    if ($isunknown(m_axi_wlast) && (m_axi_wvalid || m_axi_awvalid || wrm_state != 2'd0))
      $display("[%0t] *** ASSERTION FAIL: m_axi_wlast is X while DMA write active (wvalid=%0b awvalid=%0b state=%0d)!", $time, m_axi_wvalid, m_axi_awvalid, wrm_state);
  end
