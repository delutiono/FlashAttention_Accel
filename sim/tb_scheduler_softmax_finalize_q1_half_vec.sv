...
module tb_scheduler_softmax_finalize_q1_half_vec;
  ...
  localparam logic [L_W-1:0] L_ONE_PLUS_EXP_NEG_HALF = 32'h00cd_a2cc;
  localparam logic [31:0] RECIP_ONE_PLUS_EXP_NEG_HALF = 32'h4fac_bf4e;
  localparam logic signed [SCORE_PIPE_W-1:0] SCORE_NEG_HALF_48 = -48'sd32768;
  localparam logic signed [SOFTMAX_SCORE_W-1:0] SOFTMAX_SCORE_NEG_HALF =
      -24'sd32768;
  ...
  task automatic check_q1_second_update(input string tag);
    begin
      check_softmax_meta(tag, 8'd1, L_ONE_PLUS_EXP_NEG_HALF);
      check_softmax_lane(tag, 0, 48'sh0001_1b45_9800);
  ...
  task automatic check_q1_finalize(input string tag);
    begin
      if (q1_finalize_count == 1) begin
        ...
        if (observed_q1_recip !== RECIP_ONE_PLUS_EXP_NEG_HALF) begin
          $fatal(1, "%s observed recip_l=0x%08h expected=0x%08h",
                 tag, observed_q1_recip, RECIP_ONE_PLUS_EXP_NEG_HALF);
        end
        check_finalize_lane(tag, 0, 16'sh0161);
        check_finalize_lane(tag, 1, 16'sh0000);
        check_finalize_lane(tag, 63, 16'sh0000);
  ...
  initial begin
    ...
    if (ref_scaled_score(8'd1, 8'd0) !== '0 ||
        ref_scaled_score(8'd1, 8'd1) !== SCORE_NEG_HALF_48 ||
        ref_scaled_score(8'd1, 8'd2) !== '0) begin
      $fatal(1, "controlled q1/k scores do not match 0/-0.5/0");
    end
    ...
    for (int guard = 0; guard < 5000; guard++) begin
      ...
      if (score_valid && score_q_index == 8'd1) begin
        ...
        if (score_k_index == 8'd1 && score !== SCORE_NEG_HALF_48) begin
          $fatal(1, "q=1 k=1 score=%0d expected -32768", score);
        end

        if (score_k_index == 8'd1 &&
            softmax_score !== SOFTMAX_SCORE_NEG_HALF) begin
          $fatal(1, "q=1 k=1 softmax_score=0x%06h expected 0xff8000",
                 softmax_score);
        end
      end
      ...
      if (softmax_valid && softmax_q_index == 8'd1) begin
        ...
        if (q1_softmax_count == 1) begin
          ...
          check_q1_second_update("q1_k1_final_delta_neg_half");
          q1_hold_m = softmax_m;
          q1_hold_l = softmax_l;
      ...
      if (dut_finalize.recip_valid && q1_softmax_count == 2) begin
        ...
        if (observed_q1_recip !== RECIP_ONE_PLUS_EXP_NEG_HALF) begin
          $fatal(1, "q1 second recip_l=0x%08h expected=0x%08h",
                 observed_q1_recip, RECIP_ONE_PLUS_EXP_NEG_HALF);
        end
      end
    ...
    $display("tb_scheduler_softmax_finalize_q1_half_vec PASS q1_scores=%0d l1=0x%08h l2=0x%08h recip=0x%08h o00=0x%04h o01=0x%04h o63=0x%04h",
             q1_score_count, L_ONE, L_ONE_PLUS_EXP_NEG_HALF,
