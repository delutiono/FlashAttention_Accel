...
module tb_scheduler_gate_small;

  localparam S         = 4;
  localparam D         = 8;
  localparam BK        = 2;
  localparam ELEM_W    = 16;
  localparam DOT_LANES = 4;
...
`ifdef GATE_SIM
  fa_scheduler #(.S(S), .D(D), .BK(BK), .DOT_LANES(DOT_LANES), .V_LANES(V_LANES)) u_dut (
`else
  fa_scheduler #(.S(S), .D(D), .ELEM_W(ELEM_W), .BK(BK),
                 .DOT_LANES(DOT_LANES), .V_LANES(V_LANES),
                 .SCORE_W(SCORE_W), .EXP_W(EXP_W), .L_W(L_W), .ACC_W(ACC_W)) u_dut (
`endif
    .clk(clk), .rst_n(rst_n),
  ...
  initial begin
    ...
    $display("[%0t] Starting gate sim (S=%0d, D=%0d, BK=%0d)...", $time, S, D, BK);
    start_i = 1;
    @(posedge clk);
    start_i = 0;
