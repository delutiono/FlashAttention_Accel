...
module tb_scheduler_softmax_vec;
  ...
  task automatic check_lane(
    ...
    begin
      if (softmax_acc[lane] !== expected) begin
        $fatal(1, "%s lane%0d acc=%0d expected=%0d",
               tag, lane, softmax_acc[lane], expected);
      end
    end
  endtask

  task automatic check_q0_state(input string tag);
    begin
      if (softmax_m !== '0) begin
        $fatal(1, "%s m=%0d expected=0", tag, softmax_m);
      end

      if (softmax_l !== L_ONE) begin
        $fatal(1, "%s l=%0d expected=%0d", tag, softmax_l, L_ONE);
      end

      check_lane(tag, 0, ref_v_scaled(8'd0, 0));
      check_lane(tag, 1, ref_v_scaled(8'd0, 1));
      check_lane(tag, 2, ref_v_scaled(8'd0, 2));
      check_lane(tag, 63, ref_v_scaled(8'd0, 63));
    end
  endtask
  ...
  initial begin
    ...
    $display("tb_scheduler_softmax_vec PASS m=%0d l=%0d acc0=%0d acc1=%0d acc2=%0d acc63=%0d",
             softmax_m, softmax_l, softmax_acc[0], softmax_acc[1],
             softmax_acc[2], softmax_acc[63]);
    $finish;
  end
endmodule
