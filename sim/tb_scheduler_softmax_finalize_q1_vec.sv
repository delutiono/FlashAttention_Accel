...
module tb_scheduler_softmax_finalize_q1_vec;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      softmax_tag_valid <= score_valid && !row_start_i;
  ...
  task automatic init_controlled_vectors;
    begin
      ...
      v_mem[0 * D + 1] = 16'shff00;
      v_mem[0 * D + 63] = 16'sh0200;

      v_mem[1 * D + 0] = 16'sh0080;
      v_mem[1 * D + 1] = 16'sh0100;
      v_mem[1 * D + 63] = 16'shff00;
  ...
  task automatic check_q1_first_update(input string tag);
    begin
      ...
      check_softmax_lane(tag, 1, acc_from_q88(16'shff00));
      check_softmax_lane(tag, 63, acc_from_q88(16'sh0200));
    end
  endtask

  task automatic check_q1_second_update(input string tag);
    begin
      check_softmax_meta(tag, 8'd1, L_TWO);
      check_softmax_lane(tag, 0, 48'sh0000_c000_0000);
      check_softmax_lane(tag, 1, 48'sh0000_0000_0000);
      check_softmax_lane(tag, 63, 48'sh0000_8000_0000);
    end
  endtask
  ...
  task automatic check_q1_finalize(input string tag);
    begin
      if (q1_finalize_count == 1) begin
        ...
        check_finalize_lane(tag, 63, 16'sh0200);
      end else if (q1_finalize_count == 2) begin
        check_finalize_lane(tag, 0, 16'sh00c0);
        check_finalize_lane(tag, 1, 16'sh0000);
        check_finalize_lane(tag, 63, 16'sh0080);
  ...
  initial begin
    ...
    if (ref_scaled_score(8'd1, 8'd0) !== '0 ||
        ref_scaled_score(8'd1, 8'd1) !== '0 ||
        ref_scaled_score(8'd1, 8'd2) !== '0) begin
      $fatal(1, "controlled q1/k scores are not all zero");
    end
    ...
    for (int guard = 0; guard < 5000; guard++) begin
      ...
      if (score_valid && score_q_index == 8'd1) begin
        ...
        if (score !== '0) begin
          $fatal(1, "q=1 k=%0d score=%0d expected 0",
                 score_k_index, score);
        end
      end
      ...
      if (softmax_valid && softmax_q_index == 8'd1) begin
        ...
        if (q1_softmax_count == 1) begin
          ...
          check_q1_second_update("q1_k1_final_l2");
          q1_hold_m = softmax_m;
          q1_hold_l = softmax_l;
    ...
    $display("tb_scheduler_softmax_finalize_q1_vec PASS q1_scores=%0d l1=0x%08h l2=0x%08h o00=0x%04h o01=0x%04h o63=0x%04h",
             q1_score_count, L_ONE, L_TWO, observed_q1_o00,
