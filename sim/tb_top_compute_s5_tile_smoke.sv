...
module tb_top_compute_s5_tile_smoke;
  ...
  localparam int unsigned ROWS = 5;
  localparam int unsigned KV_TILE_ROWS = 2;
  ...
  initial begin : static_config_checks
    if (dut.COMPUTE_ROWS != ROWS) begin
      fail("top compute S5 smoke COMPUTE_ROWS override did not apply");
    end
    if (dut.KV_TILE_ROWS != KV_TILE_ROWS) begin
      fail("top compute S5 smoke KV_TILE_ROWS override did not apply");
    end
  end
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (m_axi_arvalid && m_axi_arready &&
          (((m_axi_araddr >= (K_BASE + (ROWS * STRIDE_BYTES))) &&
            (m_axi_araddr < V_BASE)) ||
           ((m_axi_araddr >= (V_BASE + (ROWS * STRIDE_BYTES))) &&
  ...
  initial begin
    ...
    $readmemh("test_vectors/generated/s5_d64_seed101/s5_d64_seed101_Q_beats64.hex", q_beats);
    $readmemh("test_vectors/generated/s5_d64_seed101/s5_d64_seed101_K_beats64.hex", k_beats);
    $readmemh("test_vectors/generated/s5_d64_seed101/s5_d64_seed101_V_beats64.hex", v_beats);
    $readmemh("test_vectors/generated/s5_d64_seed101/s5_d64_seed101_O_golden_beats64.hex",
              o_golden_beats);
    ...
    do begin
      ...
      if (status[2]) fail("top reported compute S5 tile smoke error");
      poll_count++;
      if (poll_count > 9000) fail("timeout waiting for top compute S5 tile smoke done");
    end while (!status[1]);
    ...
    for (idx = 0; idx < TOTAL_BEATS; idx++) begin
      ...
      if (got_word !== o_golden_beats[idx]) begin
        $fatal(1,
               "S5 compute O beat %0d mismatch got=0x%016x expected=0x%016x",
    ...
    $display("tb_top_compute_s5_tile_smoke PASS S=5 D=64 KV_TILE_ROWS=2");
    $finish;
  end
