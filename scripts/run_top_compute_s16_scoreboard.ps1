...
Invoke-Checked vlog "-sv" "-work" $WorkLib "-f" "rtl/filelist.f" "sim/axi_mem_model.sv" "sim/tb_top_compute_s16_multi_tile_smoke.sv"
Invoke-Checked vsim "-c" "-lib" $WorkLib "tb_top_compute_s16_multi_tile_smoke" "+DUMP_O_BEATS64=$dumpPath" "-do" "run -all; quit -f"
Invoke-Checked python "-B" "scripts/compare_vector_output.py" `
  "--metadata" "test_vectors/generated/s16_d64_seed102/s16_d64_seed102_metadata.json" `
  "--dut-hex" $dumpPath `
  "--format" "beats64" `
  "--require-mae" "0" `
  "--require-maxae" "0" `
