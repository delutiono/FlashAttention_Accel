...
// Score scheduler with configurable sequence length (num_groups = S/8).
// Supports S=256 (num_groups=32) and S=512 (num_groups=64).
...
reg [9:0] token_counter_reg;
reg group_wait_drop_reg;
reg [5:0] run_q_group_reg;
reg [5:0] run_kv_tile_reg;

wire [8:0] global_q_row = {run_q_group_reg, ctx_issue_reg};
wire [8:0] global_k_row = {run_kv_tile_reg, kv_row_reg};
...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        token_counter_reg <= 10'd0;
        group_wait_drop_reg <= 1'b0;
        run_q_group_reg <= 6'd0;
        run_kv_tile_reg <= 6'd0;
        ...
        case (state_reg)
            ...
            ST_RUN: begin
                ...
                if (issue_fire) begin
                    ...
                    token_counter_reg <= token_counter_reg + 10'd1;
                    ctx_issue_reg <= ctx_issue_reg + 3'd1;
                    if (ctx_issue_reg == 3'd7) begin
                        ctx_issue_reg <= 3'd0;
                        kv_row_reg <= kv_row_reg + 3'd1;
            ...
            ST_FIN_WAIT: begin
                if (fin_pending_reg == 8'd0) begin
                    group_done <= 1'b1;
                    if (active_q_group == num_groups - 6'd1)
                        state_reg <= ST_DONE;
                    else begin
