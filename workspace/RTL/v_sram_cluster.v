...
generate
    for (bank_idx = 0; bank_idx < 4; bank_idx = bank_idx + 1) begin : g_bank
        ...
        sky130_sram_0kbytes_1rw1r_64x64_8_wrapper u_bank_p0 (
            .clk(clk),
            .rst_n(rst_n),
            .rw_en((wide_rd_en_q && !wide_rd_addr_q[5]) || bank_selected_p0),
            .rw_write((wide_rd_en_q && !wide_rd_addr_q[5]) ? 1'b0 : rw_write),
            .rw_wmask(bank_wmask),
            .rw_addr((wide_rd_en_q && !wide_rd_addr_q[5]) ? ({1'b0, wide_rd_addr_q[4:0]} + 6'd1) : {1'b0, rw_addr[4:0]}),
            .rw_wdata(bank_wdata),
            .rw_rvalid(bank_rw_rvalid_p0[bank_idx]),
            .rw_rdata(bank_rw_rdata_p0[bank_idx]),
            .rd_en(wide_rd_en_q && !wide_rd_addr_q[5]),
            .rd_addr({1'b0, wide_rd_addr_q[4:0]}),
        ...
        sky130_sram_0kbytes_1rw1r_64x64_8_wrapper u_bank_p1 (
            .clk(clk),
            .rst_n(rst_n),
            .rw_en((wide_rd_en_q && wide_rd_addr_q[5]) || bank_selected_p1),
            .rw_write((wide_rd_en_q && wide_rd_addr_q[5]) ? 1'b0 : rw_write),
            .rw_wmask(bank_wmask),
            .rw_addr((wide_rd_en_q && wide_rd_addr_q[5]) ? ({1'b0, wide_rd_addr_q[4:0]} + 6'd1) : {1'b0, rw_addr[4:0]}),
            .rw_wdata(bank_wdata),
...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        wide_rd_en_q <= wide_rd_en;
        wide_rd_addr_q <= wide_rd_addr;
        wide_page_d1 <= wide_rd_addr[5];
        wide_page_d2 <= wide_page_d1;
        wide_page_d3 <= wide_page_d2;
    end
end
...
assign wide_rd_valid = wide_page_d3 ?
                       ((&bank_rd_valid_p1) && (&bank_rw_rvalid_p1)) :
                       ((&bank_rd_valid_p0) && (&bank_rw_rvalid_p0));
assign wide_rd_data0 = wide_page_d3 ? bank_rd_data_p1[0] : bank_rd_data_p0[0];
assign wide_rd_data1 = wide_page_d3 ? bank_rd_data_p1[1] : bank_rd_data_p0[1];
assign wide_rd_data2 = wide_page_d3 ? bank_rd_data_p1[2] : bank_rd_data_p0[2];
assign wide_rd_data3 = wide_page_d3 ? bank_rd_data_p1[3] : bank_rd_data_p0[3];
assign wide_rd_data4 = wide_page_d3 ? bank_rw_rdata_p1[0] : bank_rw_rdata_p0[0];
assign wide_rd_data5 = wide_page_d3 ? bank_rw_rdata_p1[1] : bank_rw_rdata_p0[1];
assign wide_rd_data6 = wide_page_d3 ? bank_rw_rdata_p1[2] : bank_rw_rdata_p0[2];
assign wide_rd_data7 = wide_page_d3 ? bank_rw_rdata_p1[3] : bank_rw_rdata_p0[3];
