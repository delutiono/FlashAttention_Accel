...
// dot: Q16.16, scale: unsigned Q0.16, score/m: signed Q16.16.
// alpha/beta: unsigned Q1.15. exp ROM: 16-bank, 2-stage register.
// Pipeline: m0(reg inputs) -> v0(multiply) -> v1(round/shift) -> v2(score/m) -> side+ROM
...
reg                    m0, v0, v1, v2;
reg signed [47:0]      dot_m0;
reg [15:0]             scale_m0;
reg signed [31:0]      m_old_m0;
reg [TOKEN_WIDTH-1:0]  token_m0;
reg                    mask_m0;
reg                    dropout_m0;
...
always @(posedge clk) begin
    if (!rst_n) begin
        m0 <= 1'b0; v0 <= 1'b0; v1 <= 1'b0; v2 <= 1'b0;
        mask_m0 <= 1'b0; mask_v0 <= 1'b0; mask_v1 <= 1'b0; mask_v2 <= 1'b0;
        mask_side_s0 <= 1'b0; mask_side_s1 <= 1'b0;
        dropout_m0 <= 1'b0; dropout_v0 <= 1'b0; dropout_v1 <= 1'b0; dropout_v2 <= 1'b0;
        ...
        // m0: register multiply inputs (splits 48x16 multiply retiming boundary)
        m0 <= in_valid;
        v0 <= m0;
        v1 <= v0;
        v2 <= v1;
        ...
        if (in_valid) begin
            ...
            dropout_m0 <= in_dropout_en && (lfsr[15:0] < in_dropout_prob);
        end
        if (m0) begin
            // Q16.16 * Q0.16 -> Q16.32
            product_s0 <= $signed(dot_m0) * $signed({1'b0, scale_m0});
            m_old_s0 <= m_old_m0;
            token_s0 <= token_m0;
            mask_v0 <= mask_m0;
            dropout_v0 <= dropout_m0;
        end
        if (v0) begin
            // round-to-nearest back to Q16.16
