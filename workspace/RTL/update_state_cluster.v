...
reg [1:0]              rd_quarter_pipe [0:2];
reg [2:0]              rd_context_pipe [0:2];
reg [15:0]             rd_alpha_pipe [0:2];
reg [15:0]             rd_beta_pipe [0:2];
reg [TOKEN_WIDTH-1:0]  rd_token_pipe [0:2];
reg                    rd_last_pipe [0:2];
...
reg                    mul_last_reg;
reg signed [ACC_PRODUCT_WIDTH-1:0] acc_mul_mid_reg [0:LANES-1];
reg signed [V_PRODUCT_WIDTH-1:0]   v_mul_mid_reg [0:LANES-1];
reg                    mul_mid_valid_reg;
reg [1:0]              mul_mid_quarter_reg;
reg [2:0]              mul_mid_context_reg;
reg [TOKEN_WIDTH-1:0]  mul_mid_token_reg;
reg                    mul_mid_last_reg;
reg signed [ACC_PRODUCT_WIDTH-1:0] acc_mul_reg [0:LANES-1];
reg signed [V_PRODUCT_WIDTH-1:0]   v_mul_reg [0:LANES-1];
	reg                    mul2_valid_reg;
	reg [1:0]              mul2_quarter_reg;
	reg [2:0]              mul2_context_reg;
	reg [TOKEN_WIDTH-1:0]  mul2_token_reg;
	reg                    mul2_last_reg;
	reg signed [SUM_WIDTH-1:0] mul2_acc_rnd [0:LANES-1];
	reg signed [SUM_WIDTH-1:0] mul2_v_shft [0:LANES-1];

	reg                    mul_d1_valid_reg;
	reg [1:0]              mul_d1_quarter_reg;
	reg [2:0]              mul_d1_context_reg;
	reg [TOKEN_WIDTH-1:0]  mul_d1_token_reg;
	reg                    mul_d1_last_reg;

	reg signed [ACC_WIDTH-1:0] mul_acc_data_reg [0:LANES-1];
	reg signed [15:0]          mul_v_data_reg [0:LANES-1];
	reg [15:0]                 mul_alpha_reg;
	reg [15:0]                 mul_beta_reg;
...
wire [5:0]             fin_acc_addr;
wire                   fin_acc_fire;
reg [2:0]              fin_req_pipe_valid;
reg [2:0]              fin_pair_index_pipe [0:2];
...
wire v_quarter_sel;
wire [255:0] v_rd_q;
assign v_quarter_sel = rd_quarter_pipe[2][0];
...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        mul_last_reg <= 1'b0;
        mul_mid_valid_reg <= 1'b0;
        mul_mid_quarter_reg <= 2'd0;
        mul_mid_context_reg <= 3'd0;
        mul_mid_token_reg <= {TOKEN_WIDTH{1'b0}};
        mul_mid_last_reg <= 1'b0;
		mul2_valid_reg <= 1'b0;
		mul2_quarter_reg <= 2'd0;
		mul2_context_reg <= 3'd0;
		mul2_token_reg <= {TOKEN_WIDTH{1'b0}};
		mul2_last_reg <= 1'b0;
		mul_d1_valid_reg <= 1'b0;
		mul_d1_quarter_reg <= 2'd0;
		mul_d1_context_reg <= 3'd0;
		mul_d1_token_reg <= {TOKEN_WIDTH{1'b0}};
		mul_d1_last_reg <= 1'b0;
		mul_alpha_reg <= 16'd0;
		mul_beta_reg <= 16'd0;
        sum_valid_reg <= 1'b0;
        ...
        fin_req_pipe_valid <= 3'b000;
        fin_pair_index_pipe[0] <= 3'd0;
        fin_pair_index_pipe[1] <= 3'd0;
        fin_pair_index_pipe[2] <= 3'd0;
        for (i = 0; i < LANES; i = i + 1) begin
            acc_mul_mid_reg[i] <= {ACC_PRODUCT_WIDTH{1'b0}};
            v_mul_mid_reg[i] <= {V_PRODUCT_WIDTH{1'b0}};
            acc_mul_reg[i] <= {ACC_PRODUCT_WIDTH{1'b0}};
            v_mul_reg[i] <= {V_PRODUCT_WIDTH{1'b0}};
            acc_next_reg[i] <= {ACC_WIDTH{1'b0}};
            mul_acc_data_reg[i] <= {ACC_WIDTH{1'b0}};
            mul_v_data_reg[i] <= 16'd0;
        end
        ...
        mul_valid_reg <= rd_valid_pipe[2] && acc_rd_valid && v_rd_valid;
        mul_quarter_reg <= rd_quarter_pipe[2];
        mul_context_reg <= rd_context_pipe[2];
        mul_token_reg <= rd_token_pipe[2];
        mul_last_reg <= rd_last_pipe[2];
        if (rd_valid_pipe[2] && acc_rd_valid && v_rd_valid) begin
            mul_alpha_reg <= rd_alpha_pipe[2];
            mul_beta_reg <= rd_beta_pipe[2];
            for (i = 0; i < LANES; i = i + 1) begin
                mul_acc_data_reg[i] <= acc_rd_packed[i*ACC_WIDTH +: ACC_WIDTH];
                mul_v_data_reg[i] <= v_rd_q[i*16 +: 16];
            end
        end

        if (mul_valid_reg) begin
            for (i = 0; i < LANES; i = i + 1) begin
                acc_mul_mid_reg[i] <= $signed(mul_acc_data_reg[i]) *
                                     $signed({1'b0, mul_alpha_reg});
                v_mul_mid_reg[i] <= $signed(mul_v_data_reg[i]) *
                                   $signed({1'b0, mul_beta_reg});
            end
        end

        mul_mid_valid_reg <= mul_valid_reg;
        mul_mid_quarter_reg <= mul_quarter_reg;
        mul_mid_context_reg <= mul_context_reg;
        mul_mid_token_reg <= mul_token_reg;
        mul_mid_last_reg <= mul_last_reg;
        if (mul_mid_valid_reg) begin
            for (i = 0; i < LANES; i = i + 1) begin
                acc_mul_reg[i] <= acc_mul_mid_reg[i];
                v_mul_reg[i] <= v_mul_mid_reg[i];
            end
        end

        mul_d1_valid_reg <= mul_mid_valid_reg;
        mul_d1_quarter_reg <= mul_mid_quarter_reg;
        mul_d1_context_reg <= mul_mid_context_reg;
        mul_d1_token_reg <= mul_mid_token_reg;
        mul_d1_last_reg <= mul_mid_last_reg;

        mul2_valid_reg <= mul_d1_valid_reg;
        mul2_quarter_reg <= mul_d1_quarter_reg;
        mul2_context_reg <= mul_d1_context_reg;
        mul2_token_reg <= mul_d1_token_reg;
        mul2_last_reg <= mul_d1_last_reg;
        if (mul_d1_valid_reg) begin
            for (i = 0; i < LANES; i = i + 1) begin
                mul2_acc_rnd[i] <= round_shift15(acc_mul_reg[i]);
                mul2_v_shft[i] <= $signed(v_mul_reg[i]) <<< 7;
            end
        end

        sum_valid_reg <= mul2_valid_reg;
        sum_quarter_reg <= mul2_quarter_reg;
        sum_context_reg <= mul2_context_reg;
        sum_token_reg <= mul2_token_reg;
        sum_last_reg <= mul2_last_reg;
        if (mul2_valid_reg) begin
            for (i = 0; i < LANES; i = i + 1) begin
...
`endif
        ...
        fin_req_pipe_valid[2] <= fin_req_pipe_valid[1];
        fin_pair_index_pipe[0] <= fin_acc_req_pair;
        fin_pair_index_pipe[1] <= fin_pair_index_pipe[0];
        fin_pair_index_pipe[2] <= fin_pair_index_pipe[1];
        if (fin_req_pipe_valid[2] && acc_rd_valid) begin
            fin_acc_rsp_valid <= 1'b1;
            case (fin_pair_index_pipe[2])
