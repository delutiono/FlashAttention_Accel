...
module fa_scheduler #(
  ...
  logic [2:0]              v_chunk;
  logic [1:0]              sm_wait;  // wait cycles for softmax to process score+P/alpha

  // --- quant ---
  logic signed [95:0] quant_prod;
  logic signed [S*D*ELEM_W-1:0] o_flat_int;

  // --- dot_pe ---
  logic               dot_valid_i, dot_clr_i, dot_valid_o;
  logic [2:0]         dot_chunk;
  logic signed [15:0]  q_chunk [DOT_LANES], k_chunk [DOT_LANES];
  ...
  fa_dot_pe #(.D(D), .LANES(DOT_LANES), .ACC_W(ACC_W)) u_dot (
    .clk(clk), .rst_n(rst_n),
    .valid_i(dot_valid_i), .acc_clr_i(dot_clr_i),
    .q_i(q_chunk), .k_i(k_chunk),
  ...
  fa_softmax_online #(.SCORE_W(SCORE_W), .EXP_W(EXP_W), .L_W(L_W),
    ...
    .v_i(sm_v), .v_valid_i(sm_v_valid), .last_tile_i(sm_last_tile),
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // Per-cycle debug disabled to reduce sim output
      ...
      unique case (st)
          ...
          for (int k = 0; k < BK; k++)
            if ((tile_base + k) < S)
              for (int d = 0; d < D; d++) begin
                k_buf[k][d] <= k_data_i[tile_base + k][d];
                v_buf[k][d] <= v_data_i[tile_base + k][d];
              end
          k_off <= '0;
          st    <= ST_DOT_CHUNK;
        end
        ...
        // ====================================
        ST_DOT_WAIT: begin
          // Feed remaining chunks back-to-back (don't wait for dot_valid_o)
        ...
        // ====================================
        ST_SOFTMAX: begin
          // Wait 2 cycles for softmax to process P/alpha exp, then feed V chunk 0
        ...
        // ====================================
        ST_VACC: begin
          // Feed remaining V chunks (1, 2, 3) — chunk 0 was fed in ST_SOFTMAX
          if (v_chunk < V_CHUNKS) begin
            ...
            // All V chunks fed, softmax finished accumulating
            st <= ST_NEXT_K;
          end
        end
        ...
        // ====================================
        ST_NEXT_K: begin
          if (k_off == BK - 1 || (tile_base + k_off + 1) >= S) begin
            // tile done
            st <= ST_NEXT_TILE;
          end else begin
            k_off <= k_off + 1'b1;
        ...
        // ====================================
        ST_QUANT: begin
          // O[d] = acc[d] * inv_l, quantized to Q8.8
          for (int d = 0; d < D; d++) begin
            quant_prod = $signed(sm_acc_flat[d*ACC_W +: ACC_W]) * $signed({1'b0, recip_inv_l});
            o_flat_int[(q_row * D + d)*ELEM_W +: ELEM_W] <= 16'($signed((quant_prod + (96'd1 << 39)) >>> 40));
          end
          st <= ST_NEXT_Q;
        end
  ...
  // Drive flat output from internal signal
  assign o_flat_o = o_flat_int;

  // lint helper
  logic _unused; assign _unused = neg_large_i[0];
