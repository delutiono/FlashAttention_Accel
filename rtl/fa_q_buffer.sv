...
module fa_q_buffer #(
  ...
  parameter int unsigned ROW_INDEX_W = (GROUP_ROWS <= 1) ? 1 : $clog2(GROUP_ROWS)
) (
  input  logic                         clk,
  input  logic                         rst_n,
  input  logic                         clear_i,
  ...
  localparam int unsigned ROW_COUNT_W = (GROUP_ROWS <= 1) ? 1 : $clog2(GROUP_ROWS);

  logic [BEAT_COUNT_W-1:0] beat_count_q;
  logic [ROW_COUNT_W-1:0]  q_row_count_q;
  ...
  generate
    for (bank = 0; bank < BANKS_PER_ROW; bank++) begin : gen_q_sram_bank
      ...
      assign bank_read_en = !(bank_write_en && (q_row_count_q == ROW_COUNT_W'(row_index_i)));

      /* Q group 一次缓存 8 行，score 调度时通过 row_index_i 选择当前 context 的 Q 行。 */
    ...
    if (!rst_n) begin
      ...
      if (clear_i) begin
        ...
        if (beat_count_q == BEATS_PER_ROW[BEAT_COUNT_W-1:0] - 1'b1) begin
          beat_count_q <= '0;
          if (q_row_count_q == GROUP_ROWS[ROW_COUNT_W-1:0] - 1'b1) begin
            q_row_count_q <= '0;
            load_done_o <= 1'b1;
          end else begin
            q_row_count_q <= q_row_count_q + 1'b1;
          end
        end else begin
