...
module tb_fa_top_int8_64;
  ...
  localparam INPUT_BYTES_PER_ROW = D;      // 64 int8 bytes
  localparam OUTPUT_BYTES_PER_ROW = D * 2; // 128 Q8.8 bytes
  localparam MATRIX_BYTES = S * OUTPUT_BYTES_PER_ROW;
  ...
  logic [15:0] q_scale_flat [0:(S/8)-1];
  logic [15:0] k_scale_flat [0:(S/8)-1];
  logic [15:0] v_scale_flat [0:(S/8)-1];

  // O data captured from DMA writes (Q8.8, 16-bit per element)
  logic [15:0] o_captured [0:S*D-1];
  ...
  always @(posedge clk) begin
    if (rst_n &&
        u_dut.u_core.u_update.u_v_sram.wide_rd_en &&
        u_dut.u_core.u_update.u_v_sram.rw_valid &&
        (u_dut.u_core.u_update.u_v_sram.wide_rd_addr[5] ==
         u_dut.u_core.u_update.u_v_sram.rw_addr[5])) begin
      $display("[%0t] DBG collision pm_state=%0d sched_state=%0d q_ready=%0d kv_ready=%0d active_q=%0d active_kv=%0d active_k_page=%0d active_v_page=%0d pf_state=%0d pf_tile=%0d pf_page=%0d dma_kind=%0d dma_page=%0d compute_idle=%0d score_valid=%0d score_ready=%0d update_ready=%0d issue_active=%0d",
               $time,
               u_dut.u_page.state_reg,
               u_dut.u_sched.state_reg,
               u_dut.q_group_ready,
               u_dut.kv_tile_ready,
               u_dut.active_q_group,
               u_dut.active_kv_tile,
               u_dut.active_k_page,
               u_dut.active_v_page,
               u_dut.u_page.prefetch_state_reg,
               u_dut.u_page.prefetch_tile_reg,
               u_dut.u_page.prefetch_page_reg,
               u_dut.dma_rd_kind,
               u_dut.dma_rd_page,
               u_dut.compute_idle,
               u_dut.score_valid,
               u_dut.score_ready,
               u_dut.u_core.update_ready,
               u_dut.u_core.u_update.issue_active_reg);
    end
  end
  ...
  localparam [11:0] REG_LOWP_CFG = 12'h060;
  localparam [11:0] REG_LOWP_SCALE_ADDR = 12'h07C;
  localparam [11:0] REG_LOWP_SCALE_DATA = 12'h080;
  ...
  initial begin
    ...
    $display("[%0t] Loading INT8 Q/K/V test vectors...", $time);
    $readmemh("sim/int8_64_q_i8.hex", q_flat);
    for (row = 0; row < S; row++) begin
      for (col = 0; col < D; col++) begin
        idx = row * D + col;
        byte_addr = Q_BASE + row * INPUT_BYTES_PER_ROW + col;
        mem[byte_addr] = q_flat[idx];
      end
    end
    $readmemh("sim/int8_64_k_i8.hex", k_flat);
    for (row = 0; row < S; row++) begin
      for (col = 0; col < D; col++) begin
        idx = row * D + col;
        byte_addr = K_BASE + row * INPUT_BYTES_PER_ROW + col;
        mem[byte_addr] = k_flat[idx];
      end
    end
    $readmemh("sim/int8_64_v_i8.hex", v_flat);
    for (row = 0; row < S; row++) begin
      for (col = 0; col < D; col++) begin
        idx = row * D + col;
        byte_addr = V_BASE + row * INPUT_BYTES_PER_ROW + col;
        mem[byte_addr] = v_flat[idx];
      end
    end
    $readmemh("sim/int8_64_q_scale.hex", q_scale_flat);
    $readmemh("sim/int8_64_k_scale.hex", k_scale_flat);
    $readmemh("sim/int8_64_v_scale.hex", v_scale_flat);
    ...
    axi_write(REG_SEQ_LEN, 32'd8);  // S=64 => num_groups=8
    axi_write(REG_LOWP_CFG, 32'h0000_0801); // INT8_BLOCK, block_rows=8
    for (block = 0; block < S/8; block = block + 1) begin
      axi_write(REG_LOWP_SCALE_ADDR, {24'd0, 2'd0, block[5:0]});
      axi_write(REG_LOWP_SCALE_DATA, {16'd0, q_scale_flat[block]});
      axi_write(REG_LOWP_SCALE_ADDR, {24'd0, 2'd1, block[5:0]});
      axi_write(REG_LOWP_SCALE_DATA, {16'd0, k_scale_flat[block]});
      axi_write(REG_LOWP_SCALE_ADDR, {24'd0, 2'd2, block[5:0]});
      axi_write(REG_LOWP_SCALE_DATA, {16'd0, v_scale_flat[block]});
    end
    ...
    for (row = 0; row < S; row++) begin
      for (col = 0; col < D; col++) begin
        idx = row * D + col;
        byte_addr = O_BASE + row * OUTPUT_BYTES_PER_ROW + col * 2;
        o_captured[idx] = {mem[byte_addr+1], mem[byte_addr]};
      end
    end

    // Dump O to hex
    fd = $fopen("sim/o_tb_int8_64.hex", "w");
    if (fd) begin
      for (i = 0; i < S * D; i++)
        $fwrite(fd, "%04h\n", o_captured[i]);
      $fclose(fd);
      $display("  o_tb_int8_64.hex written (%0d entries).", S * D);
    end else begin
      $display("  ERROR: Cannot write o_tb_int8_64.hex");
    end
  ...
  // =============================================================
  initial begin
    #200000000;  // 200ms simulated timeout (safety net)
    $finish;
  end
