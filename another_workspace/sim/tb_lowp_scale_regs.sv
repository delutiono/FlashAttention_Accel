...
module tb_lowp_scale_regs;
  ...
  axi_lite_regs dut (
    ...
    .causal_enable(), .format_sel(), .q_base_addr(), .k_base_addr(),
    .v_base_addr(), .o_base_addr(), .stride_bytes(), .valid_len(),
    .seq_len(), .neg_large(), .score_scale(),
    .task_chain_enable(), .task_queue_not_empty(), .task_dequeue(1'b0),
    .task_busy(1'b0), .task_done(1'b0), .task_error(1'b0),
    .perf_read_data(32'd0), .stream_en(), .dropout_en(),
    .dropout_seed(), .dropout_prob(), .num_heads(), .head_stride(),
