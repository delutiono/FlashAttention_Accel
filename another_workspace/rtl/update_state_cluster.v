...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        // MUL stage: register operands (splits SRAM read -> multiply timing path)
        if (rd_valid_pipe[1] && acc_rd_valid && v_rd_valid) begin
            mul_alpha_reg <= rd_alpha_pipe[1];
            mul_beta_reg <= rd_beta_pipe[1];
            for (i = 0; i < LANES; i = i + 1) begin
        ...
        // MUL_MID stage: multiply from registered operands
        ...
        if (mul_valid_reg) begin
            for (i = 0; i < LANES; i = i + 1) begin
                acc_mul_reg[i] <= $signed(mul_acc_data_reg[i]) *
                                  $signed({1'b0, mul_alpha_reg});
                v_mul_reg[i] <= $signed(mul_v_data_reg[i]) *
                                $signed({1'b0, mul_beta_reg});
            end
        end

        sum_valid_reg <= mul_mid_valid_reg;
        sum_quarter_reg <= mul_mid_quarter_reg;
        sum_context_reg <= mul_mid_context_reg;
        sum_token_reg <= mul_mid_token_reg;
        sum_last_reg <= mul_mid_last_reg;
