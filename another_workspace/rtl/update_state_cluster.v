...
//
// P0 timing: SRAM read data captured into dedicated registers before the
// 48x17 / 16x17 multipliers, removing SRAM access delay from the multiply path.
// P2 timing: round_shift15 + v_mul<<7 sum separated from sat_acc,
// so neither the 66-bit adder nor the saturation comparator share a cycle.
module update_state_cluster #(
	...
	// P0: SRAM data capture stage -- isolates SRAM access from multiply.
	reg                    cap_valid_reg;
	reg                    cap_half_reg;
	reg [2:0]              cap_context_reg;
	reg [TOKEN_WIDTH-1:0]  cap_token_reg;
	reg                    cap_last_reg;
	reg [15:0]             cap_alpha_reg;
	reg [15:0]             cap_beta_reg;
	reg signed [ACC_WIDTH-1:0] cap_acc_reg [0:LANES-1];
	reg signed [15:0]      cap_v_reg [0:LANES-1];
	...
	// P2: raw sum stage (round+add only, no saturation).
	reg                    sum_raw_valid_reg;
	reg                    sum_raw_half_reg;
	reg [2:0]              sum_raw_context_reg;
	reg [TOKEN_WIDTH-1:0]  sum_raw_token_reg;
	reg                    sum_raw_last_reg;
	reg signed [SUM_WIDTH-1:0] sum_raw_reg [0:LANES-1];

	// P2: saturation stage (sat_acc only) -- replaces old sum_valid_reg.
	reg                    sum_sat_valid_reg;
	reg                    sum_sat_half_reg;
	reg [2:0]              sum_sat_context_reg;
	reg [TOKEN_WIDTH-1:0]  sum_sat_token_reg;
	reg                    sum_sat_last_reg;
	...
	assign idle = !init_active_reg && !issue_active_reg &&
	              (rd_valid_pipe == 2'b00) && !cap_valid_reg &&
	              !mul_valid_reg && !sum_raw_valid_reg && !sum_sat_valid_reg &&
	...
	assign acc_wr_en = init_active_reg || sum_sat_valid_reg;
	assign acc_wr_addr = init_active_reg ? {init_context_reg, init_half_reg} :
	                                      {sum_sat_context_reg, sum_sat_half_reg};
	...
	always @(posedge clk) begin
	    if (!rst_n) begin
	        ...
	        cap_valid_reg <= 1'b0;
	        cap_half_reg <= 1'b0;
	        cap_context_reg <= 3'd0;
	        cap_token_reg <= {TOKEN_WIDTH{1'b0}};
	        cap_last_reg <= 1'b0;
	        cap_alpha_reg <= 16'd0;
	        cap_beta_reg <= 16'd0;
	        mul_valid_reg <= 1'b0;
	        ...
	        mul_last_reg <= 1'b0;
	        sum_raw_valid_reg <= 1'b0;
	        sum_raw_half_reg <= 1'b0;
	        sum_raw_context_reg <= 3'd0;
	        sum_raw_token_reg <= {TOKEN_WIDTH{1'b0}};
	        sum_raw_last_reg <= 1'b0;
	        sum_sat_valid_reg <= 1'b0;
	        sum_sat_half_reg <= 1'b0;
	        sum_sat_context_reg <= 3'd0;
	        sum_sat_token_reg <= {TOKEN_WIDTH{1'b0}};
	        sum_sat_last_reg <= 1'b0;
	        ...
	        // Meta/L recurrence path (unchanged).
	        ...
	        // L1: multiplier-only stage (unchanged).
	        ...
	        // L2: rounding, beta alignment, addition and saturation (unchanged).
	        ...
	        // P0: SRAM data capture stage -- isolate SRAM delay from multiply.
	        cap_valid_reg <= rd_valid_pipe[1] && acc_rd_valid && v_rd_valid;
	        cap_half_reg <= rd_half_pipe[1];
	        cap_context_reg <= rd_context_pipe[1];
	        cap_token_reg <= rd_token_pipe[1];
	        cap_last_reg <= rd_last_pipe[1];
	        cap_alpha_reg <= rd_alpha_pipe[1];
	        cap_beta_reg <= rd_beta_pipe[1];
	        if (rd_valid_pipe[1] && acc_rd_valid && v_rd_valid) begin
	            for (i = 0; i < LANES; i = i + 1) begin
	                cap_acc_reg[i] <= $signed(acc_rd_packed[i*ACC_WIDTH +: ACC_WIDTH]);
	                cap_v_reg[i]   <= $signed(v_rd_packed[i*16 +: 16]);
	            end
	        end

	        // Multiply stage: 48x17 and 16x17 (inputs from cap_* registers).
	        mul_valid_reg <= cap_valid_reg;
	        mul_half_reg <= cap_half_reg;
	        mul_context_reg <= cap_context_reg;
	        mul_token_reg <= cap_token_reg;
	        mul_last_reg <= cap_last_reg;
	        if (cap_valid_reg) begin
	            for (i = 0; i < LANES; i = i + 1) begin
	                acc_mul_reg[i] <= cap_acc_reg[i] *
	                                  $signed({1'b0, cap_alpha_reg});
	                v_mul_reg[i]   <= cap_v_reg[i] *
	                                  $signed({1'b0, cap_beta_reg});
	            end
	        end

	        // P2: Raw sum stage -- round_shift15 + v_mul<<7, no saturation.
	        sum_raw_valid_reg <= mul_valid_reg;
	        sum_raw_half_reg <= mul_half_reg;
	        sum_raw_context_reg <= mul_context_reg;
	        sum_raw_token_reg <= mul_token_reg;
	        sum_raw_last_reg <= mul_last_reg;
	        if (mul_valid_reg) begin
	            for (i = 0; i < LANES; i = i + 1) begin
	                sum_raw_reg[i] <= round_shift15(acc_mul_reg[i]) +
	                                  ($signed(v_mul_reg[i]) <<< 7);
	            end
	        end

	        // P2: Saturate stage -- sat_acc only.
	        sum_sat_valid_reg <= sum_raw_valid_reg;
	        sum_sat_half_reg <= sum_raw_half_reg;
	        sum_sat_context_reg <= sum_raw_context_reg;
	        sum_sat_token_reg <= sum_raw_token_reg;
	        sum_sat_last_reg <= sum_raw_last_reg;
	        if (sum_raw_valid_reg) begin
	            for (i = 0; i < LANES; i = i + 1) begin
	                acc_next_reg[i] <= sat_acc(sum_raw_reg[i]);
	            end
	        end

	        // ACC half1 write, meta write and completion commit together.
	        if (sum_sat_valid_reg && sum_sat_half_reg) begin
	            meta_wr_en <= 1'b1;
	            meta_wr_addr <= {1'b0, sum_sat_context_reg};
	            meta_wr_data <= {48'd0, pending_l[sum_sat_context_reg], pending_m[sum_sat_context_reg]};
	            complete_valid <= 1'b1;
	            complete_context <= sum_sat_context_reg;
	            complete_token <= sum_sat_token_reg;
	            complete_last <= sum_sat_last_reg;
	            complete_m <= pending_m[sum_sat_context_reg];
	            complete_l <= pending_l[sum_sat_context_reg];
	            pending_meta_valid_reg[sum_sat_context_reg] <= 1'b0;
	`ifndef SYNTHESIS
	            if (!pending_meta_valid_reg[sum_sat_context_reg]) begin
