...
module fa_softmax_online #(
  ...
  input  logic signed [16*V_ACC_LANES-1:0] v_i,
  ...
  output logic signed [ACC_W*D-1:0]    acc_o,
  output logic                         done_o,
  output logic signed [16*D-1:0]       o_elem_o,
  ...
  logic [L_W-1:0]            l_q;
  logic signed [ACC_W*D-1:0] acc_q;
  ...
  logic [47:0] l_scaled;
  logic signed [ACC_W*D-1:0] acc_scaled_q;
  ...
  always_comb begin
    for (acc_i = 0; acc_i < D; acc_i = acc_i + 1) begin
      acc_scaled_q[acc_i*ACC_W +: ACC_W] = alpha_needed
        ? 48'((64'(acc_q[acc_i*ACC_W +: ACC_W]) * 64'($signed({1'b0, alpha_val}))) >>> 16)
        : acc_q[acc_i*ACC_W +: ACC_W];
    end
  end
  ...
  always_comb begin
    for (pi = 0; pi < D; pi = pi + 1)
      acc_o_flat[pi*ACC_W +: ACC_W] = acc_q[pi*ACC_W +: ACC_W];
  end

  // Also drive acc_o (packed port)
  assign acc_o = acc_q;
        ...
        ST_ACCUM: begin
          if (v_valid_i) begin
            for (int i = 0; i < V_ACC_LANES; i++) begin
              ...
              V_s = $signed(v_i[i*16 +: 16]);
              vacc_prod = P_s * V_s;
              acc_q[(v_cnt * V_ACC_LANES + i)*ACC_W +: ACC_W]
                <= acc_q[(v_cnt * V_ACC_LANES + i)*ACC_W +: ACC_W] + (ACC_W'(vacc_prod) <<< 8);
            end
