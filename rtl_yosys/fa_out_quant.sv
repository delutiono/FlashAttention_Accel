...
module fa_out_quant #(
  ...
  input  logic                         valid_i,
  input  logic signed [ACC_W*LANES-1:0] acc_i,
  input  logic        [INV_W-1:0]      inv_l_i,
  input  logic                         last_i,

  output logic                         ready_o,
  output logic                         valid_o,
  output logic signed [OUT_W*LANES-1:0] o_o
);
  ...
  logic signed [ACC_W*LANES-1:0] acc_r;
  logic        [INV_W-1:0]       inv_l_r;

  // Pipeline stage 1: register inputs, start multiply
  logic               st1_valid;
  logic signed [ACC_W*LANES-1:0] acc_s1;
  logic signed [96*LANES-1:0] mul_s1;  // ACC_W + INV_W bits

  logic               st2_valid;
  logic signed [96*LANES-1:0] mul_s2;
    ...
    if (!rst_n) begin
      ...
      if (active) begin
        ...
        for (int i = 0; i < LANES; i++) begin
          ...
          a = $signed({acc_r[i*ACC_W + (ACC_W-1)], acc_r[i*ACC_W +: ACC_W]});
        ...
        if (st1_valid) begin
          ...
          if (st2_valid) begin
            for (int i = 0; i < LANES; i++) begin
              ...
              rounded = $signed(mul_s2[i*96 +: 96]) + (96'd1 << 39);
              o_o[i*OUT_W +: OUT_W] <= ($signed(rounded >>> 40) > Q88_MAX) ? Q88_MAX :
